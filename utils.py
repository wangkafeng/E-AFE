	...
	def exposed_evaluate(self, X, d, task, i_model, i_evl):
		...
		if task == 'regression':
			...
			if i_model == 'RF':
				...
				model = svm.SVC()
			# r = cross_val_score(model, X, y, scoring='accuracy', cv=5).mean()
			if i_evl == 'f_score':
				# print(evl.f_measure(0))
				# return evl.f_measure(0)
...
def init_name_and_log(args):
	...
	# logging.basicConfig(filename='log/' + name + '.log', level=logging.INFO)
	logging.basicConfig(filename='new_log/' + name + '.log', level=logging.INFO)
	logging.info('--start--')
	return name
...
if __name__ == '__main__':
	try:
		jvm.start(max_heap_size='5g')  # kafeng modify, move into try: 
